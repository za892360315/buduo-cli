...
export const templates: Map<string, TemplateInfo> = new Map([
    [
        'Vite-Vue3-Typescript-tempalte',
        {
            ...
            downloadUrl: 'https://gitee.com/zhang-haijing1/admin-pro.git',
...
export const checkVersion = async (name: string, version: string) => {
    ...
    if (need) {
        console.warn(
            `检查到buduo最新版本： ${chalk.blackBright(latestVersion)}，当前版本是：${chalk.blackBright(version)}`
        );
        console.log(
            `可使用： ${chalk.yellow('npm install buduo-cli@latest')}，或者使用：${chalk.yellow('buduo update')}更新`
        );
    }
    return need;
};
